...
---
### 补充中间件功能：上传图片时将 userID 存入数据库

权限控制+溯源链条（LSB 水印中虽然会嵌入 userID，但这个 userID 是在下载时才嵌入的。）+业务逻辑
controllers\image_controller.go：   GetLsbWatermarkedImage 方法：

```
func (ctrl *ImageController) GetLsbWatermarkedImage(c *gin.Context) {
	imageID := c.Param("image_id")
	// 实际应用中 userID 从 JWT 中间件获取
...
验证：“这个操作者是否有权限处理 imageID 这张图片？” 要回答这个问题，数据库里必须存有图片和其所有者（userID）的关联关系。

1.  **在type EncryptedImage struct里面存储userID**:

UserID           string         `gorm:"type:varchar(64);not null;index"` // <-- 【新增】用户ID，并建立索引

2.  **重构上传服务 (services/image_upload_service.go)**:
ImageUploadService 现在需要接收 UserID 作为参数，并将其存入数据库。

3.  **重构上传控制器 (controllers/image_upload_controller.go)**:
控制器现在需要从 Gin Context 中提取 userID，并将其传递给服务。

4.  **在获取服务中加入权限检查——ImageService。**:
   GetDecryptedImageData

当前的设计就是 **“只有上传者有资格加LSB水印”**——资源所有权模型。

后续可以扩展： **引入一个新的“权限服务 (Authorization Service)”或在 ImageService 中引入更复杂的逻辑。**
